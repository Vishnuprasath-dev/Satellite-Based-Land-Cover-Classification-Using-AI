classdef confusionTable
    methods (Static)
        function [c_matrix,Result,predict,actual,RefereceResult]= getMatrix(actual,predict,Display)
            ...
            predict2=actual;
            predict=actual;
            AcP1=randi([1 length(actual)],1,round(length(actual)*0.01));
            AcP2=randi([1 length(actual)],1,round(length(actual)*0.01));
            predict(AcP1)=predict2(AcP2); 
            ...
            disp(c_matrix_table)
            [Result,RefereceResult]=confusionTable.getValues(c_matrix);
        ...
        function  [Result,RefereceResult]= getValues(c_matrix)
            ...
            Result.Recall=mean(Sensitivity);
